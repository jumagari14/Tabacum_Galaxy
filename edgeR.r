...
if (!is.null(opt$filesPath)) {
  ...
  counts <- counts[rowSums(counts[,-1:-2])>0,]
  counts[,-1]-> counts
...
histOut <- makeOut("hist_pvalues.png")
...
for (i in 1:length(contrastData)) {
  ...
  top <- topTags(res, adjust.method=opt$pAdjOpt, n=Inf, sort.by="p.value")
  write.csv(top, file="results_edgeR.csv")
  ...
  # top <- gsub(",", " ", top)   # remove comma
  # png(histOut,500,500)
  # hist(as.numeric(top[-1,14]),breaks = 50,col = "grey")
  # linkName <- "Histogram"
  # linkAddr <- "hist_pvalues.png"
  # linkData <- rbind(linkData, c(linkName, linkAddr))
  # invisible(dev.off())
